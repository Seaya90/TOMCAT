...
--%>
	...
	<h2>JSTL 사용</h2>	
	<%-- 속성(변수) 선언태그(set) : setAttribute 역할 
		<c:set var="속성(변수)이름" value="값" scope="범위"></c:set>
	...
	<%-- 데이타 출력태그(out) : 표현식이나 EL 역할 
		<c:out value="출력할 값 or 변수사용시 EL방식"></c:out>
	--%>
	<p><c:out value="Hello World!!!"/></p>
	<p><c:out value="${test }" default="Hello~"/></p>
	<p>EL : ${test }</p>
	<p>EL(page) : ${pageScope.test }</p>
	<p>EL(request) : ${requestScope.test }</p>

	<%-- 삭제태그(remove) : 선언된 변수 삭제(removeAttribute 역할 
		주의 : scope 선언이 없으면 모든 scope 속성 삭제
	...
	<h2>choose문 : 자바의 switch문과 비슷(if ~ else 처리 가능)</h2>
	<%-- 
		<c:choose>
	...
	<c:set var="jumsu" value="59"/>
	<c:if test="${jumsu >= 60 }">
		<p>판정결과(${jumsu }점) : <c:out value="합격"/></p>
	</c:if>
	<c:if test="${jumsu < 60 }">
		<p>판정결과(${jumsu }점) : 불합격</p>
	</c:if>
	<hr>
	...
	<%-- choose문을 사용하고 평가결과를 저장한 testResult가
		...
		출력형태> 평가결과(95점) : A학점  
	--%>
	<c:set var="testResult" value="90"/>
	<c:choose>
		<c:when test="${testResult >= 90}">
			평가결과(${testResult}점) : A학점
		</c:when>
		<c:when test="${testResult >= 80}">
			평가결과(${testResult}점) : B학점
		</c:when>
		<c:when test="${testResult >= 70}">
			평가결과(${testResult}점) : C학점
		</c:when>
		<c:otherwise>
			평가결과(${testResult}점) : 재수강
		</c:otherwise>
	</c:choose>
	<hr><hr>
	...
	<%-- ==================================== --%>
	<h2>반복문 : forEach문</h2>
	<h3>forEach문 : 자바의 for (int i=1; i&lt;=10; i++) 형식 사용</h3>
	<%-- 
	...
	<p>1~10까지의 수 중에 짝수 출력(실습 step="2" 적용)</p>
